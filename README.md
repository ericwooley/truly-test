# Truly Code Challenge
This was originally based on the create react typescript app, see below for the original readme.

## Additional Tech added
* Storybooks
* Storyshots
* redux
* redux-sagas
...
## Running the app.
1. `yarn install`
1. run `yarn dev` and everything will start. 
1. open [storybooks](http://localhost:6006) and the [web pack dev server](http://localhost:3000) 
1. (optional) run `yarn test` in a second terminal, to have the tests watch and run on file change.
## Project Layout
``` bash 
src
├── App.css # all css goes here
├── App.story.tsx # story for the app
├── App.tsx # main app file, where store and MUI are hooked up.
├── HOC ## folder for higher order components.
├── components # most the actual react code is in this folder. 
│   ├── appBar # each sub folder contains a story and a component
│   │   ├── appBar.story.tsx
│   │   └── appBar.tsx
│   ├── ... 
│   └── stories.tsx # imports each component so you don't have to keep going to /stories/index.js
├── config # this is where config stuff goes
│   └── config.ts
├── containers # smart components who are permanently hooked up to some state/action creators, consumes components
├── index.tsx # bootstraps the react app
├── interfaces # interfaces for data within the app, only contacts for this one.
│   └── contacts.ts
├── redux # redux folder, contains the store, and ducks
│   ├── actions.ts # shortcut for getting to all actions
│   ├── contacts # all contact related files are in here
│   │   ├── __tests__ # tests for contact files
│   │   │   ├── contacts.sagas.test.ts # test for the contacts saga
│   │   │   └── contacts.selectors.test.ts # test for contact selectors
│   │   ├── actions.ts # all actions are in here, which helps avoid cyclical dependencies between reducers and sagas
│   │   ├── reducer.ts # the actual reducer is in here. 
│   │   ├── saga.ts # handles all async tasks and side affects
│   │   └── selectors.ts # functions which, when given the state, return some helpful info
│   ├── selectors.ts # shortcut for all selectors
│   ├── store.ts # The store, and middle ware are produced here.
│   └── ui # another duck, like contacts
├── registerServiceWorker.ts # came with CRA, registers a service worker for offline access.
├── services # Services that call out to the api, or write to disk or wahtever
│   └── getContacts.ts
├── setupTests.ts # Setup pollyfills etc.. for testss
└── storyshots # houses all the snapshot testing for all the storybooks
    ├── __snapshots__
    │   └── storyshots.test.tsx.snap
    └── storyshots.test.tsx
```
